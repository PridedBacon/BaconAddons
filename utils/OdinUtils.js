...
export function swapAndRightClick(items, swapBack = true) {
    index = Player?.getInventory()
        ?.getItems()
        ?.findIndex((item) => item?.getName()?.includes(items));
    if (index < 0 || index > 8) {
...
export function noSqrt3DDistance(mcEntity1, mcEntity2) {
    return (
        Math.pow(mcEntity1.field_70165_t - mcEntity2.field_70165_t, 2) +
...
export function getPhase() {
    ...
    if (
        Dungeon.inDungeon &&
        isCoordinateInsideBox({ x: Player.getX(), y: Player.getY(), z: Player.getZ() }, corner1, corner2)
    ) {
        inBoss = true;
        if (Player.getY() > 210) {
            inPhase = "p1";
...
const c = new ChatComponentText(
    "§cYou are temporarily banned for §f 29d 23h 59m 59s §cfrom this server!\n\n§7Reason: §rCheating through the use of unfair game advantages.\n§7Find out more: §b§nhttps://www.hypixel.net/appeal\n\n§7Ban ID: §r#" +
        makeid() +
 ...
 */
export function rect(r, g, b, a, x, y, width, height) {
 ...
 * @param {number} a Alpha
 */
export function centeredString(thefont, text, x, y, r, g, b, a) {
    thefont.drawStringWithShadow(
        text,
        x + (70 - thefont.getWidth(text)) / 2 + 15,
        y + 5,
 ...
 * @param {number} a Alpha
 */
export function normalString(thefont, text, x, y, r, g, b, a) {
    thefont.drawStringWithShadow(text, x, y + 5, new java.awt.Color(r, g, b, a));
}

/**
 * Plays a gui clicking sound
 */
export function makePressSound() {
...
export function drawDesc(mx, my, tab, index) {
    ...
    if (tab.x < 500) {
        ...
        rect(
            ...
            45,
            255,
            tab.x - fontmc.getWidth(description) - 4,
            my - 5,
...
export function windowClick(itemIndex, clickType) {
    Client.getMinecraft().field_71442_b.func_78753_a(
        Player.getPlayer().field_71069_bz.field_75152_c,
        itemIndex,
        Player.getHeldItemIndex(),
        clickType,
        Player.getPlayer()
    );
}
...
export function isVecInYZ(vec, aabb) {
    return (
        vec != null &&
        vec.field_72448_b >= aabb.field_72338_b &&
        vec.field_72448_b <= aabb.field_72337_e &&
        vec.field_72449_c >= aabb.field_72339_c &&
        vec.field_72449_c <= aabb.field_72334_f
    );
}
...
export function isVecInXZ(vec, aabb) {
    return (
        vec != null &&
        vec.field_72450_a >= aabb.field_72340_a &&
        vec.field_72450_a <= aabb.field_72336_d &&
        vec.field_72449_c >= aabb.field_72339_c &&
        vec.field_72449_c <= aabb.field_72334_f
    );
}
